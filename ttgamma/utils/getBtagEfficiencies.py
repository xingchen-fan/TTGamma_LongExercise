...
btagEff_etaBins = np.arange(0, 2.4001, 0.6)
...
class BjetEfficiencies(processor.ProcessorABC):
    ...
    def __init__(self):
        ...
        self._accumulator = processor.dict_accumulator(
            {
                "hJets": hist.Hist(
                    "Counts", dataset_axis, jetPt_axis, jetEta_axis, jetFlav_axis
                ),
                "hBJets": hist.Hist(
                    "Counts", dataset_axis, jetPt_axis, jetEta_axis, jetFlav_axis
                ),
    ...
    def process(self, df):
        ...
        jetSelect = (
            (jets.pt > 30) & (abs(jets.eta) < 2.4) & ((jets.jetId >> 0 & 1) == 1)
        )
...
f = {
    "QCD_Pt20to30_Ele_2016": fileSet_2016["QCD_Pt20to30_Ele_2016"],
...
for k in l_total.keys():
    ...
    if 0 in l_total[k]:
        print(k, "light", l_total[k])
    if 0 in l_total[k]:
        print(k, "charm", c_total[k])
    if 0 in l_total[k]:
        print(k, "b", b_total[k])
...
for k in l_total.keys():
    btagEff.append(
        np.array(
            [
                l_tagged[k] / l_total[k],
                c_tagged[k] / c_total[k],
